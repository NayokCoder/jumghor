...
const ProductImage = ({ toy }) => {
  ...
  const images = toy.image_urls || [toy.image_url];
    ...
    <div className="space-y-6">
      ...
      <div className="relative group">
        <div className="aspect-square bg-gradient-to-br from-white via-slate-50 to-blue-50 rounded-2xl shadow-2xl overflow-hidden border border-slate-200/60 backdrop-blur-sm">
          <Image
            src={images[selectedImageIndex] || "/placeholder-toy.jpg"}
            alt={toy.name}
            width={700}
            height={700}
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all duration-300 flex items-center justify-center">
            <button
              ...
              className="opacity-0 group-hover:opacity-100 transition-all duration-300 bg-white/90 backdrop-blur-sm text-gray-800 px-4 py-2 rounded-full font-medium shadow-lg hover:bg-white hover:shadow-xl transform hover:scale-105"
            >
              {isZoomed ? 'Reset' : 'Zoom'}
            </button>
          </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {images.map((image, index) => (
            <button
              ...
              className={`relative w-24 h-24 rounded-xl overflow-hidden border-3 transition-all duration-300 flex-shrink-0 ${
    ...
    <div className="space-y-8 bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <span className="inline-block px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-sm font-semibold rounded-full shadow-md">
            {toy.category || 'Toys & Games'}
          </span>
          ...
          {toy.original_price && toy.original_price > toy.price && (
            <div className="flex flex-col">
              ...
              <span className="text-sm text-emerald-600 font-semibold">
                {Math.round(((toy.original_price - toy.price) / toy.original_price) * 100)}% OFF
              </span>
            </div>
          )}
        </div>
        {toy.original_price && toy.original_price > toy.price && (
          <div className="flex items-center gap-2">
            <span className="text-lg font-semibold text-red-600">
              Save ${(toy.original_price - toy.price).toFixed(2)}
            </span>
      ...
      <div className={`flex items-center gap-3 p-4 rounded-xl border-2 ${
        ...
        <div className={`w-4 h-4 rounded-full ${
          toy.stock_quantity > 0 ? 'bg-green-500' : 'bg-red-500'
        }`}></div>
        <span className={`font-semibold ${
          toy.stock_quantity > 0 ? 'text-green-700' : 'text-red-700'
        }`}>
          {toy.stock_quantity > 0 ? (
            <>
      ...
      {toy.stock_quantity > 0 && (
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center bg-gray-100 rounded-xl">
            ...
            <button
              onClick={() => setQuantity(Math.min(toy.stock_quantity, quantity + 1))}
          ...
          <span className="text-gray-500 text-lg font-semibold">
            Total: ${(toy.price * quantity).toFixed(2)}
          </span>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div className="flex gap-3">
          <button 
            ...
            disabled={toy.stock_quantity === 0}
            className={`flex-1 font-semibold py-4 px-8 rounded-xl transition-all duration-300 flex items-center justify-center gap-3 shadow-lg ${
              showSuccess
          ...
          <button 
            ...
            className={`p-4 rounded-xl border-2 transition-all duration-300 shadow-md hover:shadow-lg ${
              isFavorite 
        ...
        <button className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-[1.02]">
        ...
        {activeTab === 'description' && (
          <div className="prose max-w-none">
            ...
            <p className="text-gray-600 leading-relaxed">
              {toy.description || 'This amazing toy provides hours of entertainment and educational value. Crafted with care and attention to detail, it\'s perfect for children of all ages. Made from high-quality, safe materials, this toy meets all safety standards and is built to last.'}
            </p>
...
export default function ProductPage({ toy }) {
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8 border border-slate-200/60">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link 
                href="/all-toys"
                className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-105"
              >
              ...
              <div className="flex items-center gap-2 text-sm text-gray-500">
                ...
                <Link href="/all-toys" className="hover:text-indigo-600 font-medium transition-colors">All Toys</Link>
                <span className="text-gray-400">•</span>
